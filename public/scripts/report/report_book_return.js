...
$(function(){
    $('#list_return').load(baseUrl + '/report_book_return/content');
});
...
function search(){
    ...
    if(fromdate.length != 0 || todate.length != 0){
        if(fromdate.length != 0 && todate.length != 0){ 
            ...
            if(date3 <= date4){
                $('#list_return').load(baseUrl + '/report_book_return/content?page=1&fromdate='+fromdate+'&todate='+todate+'&title='+title+'&status='+status);
...
function view_page_return(pages){
    page = pages;
    $('#list_return').load(baseUrl + '/report_book_return/content?page='+page+'&fromdate='+fromdate+'&todate='+todate+'&title='+title+'&status='+status);
}

function export_xlsx(){
    window.open(baseUrl + '/report_book_return/export_xlsx?fromdate='+fromdate+'&todate='+todate+'&title='+title+'&status='+status);
}
