...
$(function(){
    $('#list_import').load(baseUrl + '/report_device_imp/content');
});
...
function view_page_import(){
    $('#list_import').load(baseUrl + '/report_device_imp/content?page='+page+'&fromdate='+fromdate+'&todate='+todate+'&source='+source);
}
...
function search(){
    ...
    if(fromdate.length != 0 || todate.length != 0){
        if(fromdate.length != 0 && todate.length != 0){
            ...
            if(date1 <= date2){
                $('#list_import').load(baseUrl + '/report_device_imp/content?page=1&fromdate='+fromdate+'&todate='+todate+'&source='+source);
