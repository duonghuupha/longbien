...
class Subject_Model extends Model{
    ...
    function getFetObj($offset, $rows){
        $result = array();
        $query = $this->db->query("SELECT COUNT(*) AS Total FROM tbldm_subject WHERE status = 0");
        $row = $query->fetchAll();
        $query = $this->db->query("SELECT id, title, create_at, set_point FROM tbldm_subject WHERE status = 0
