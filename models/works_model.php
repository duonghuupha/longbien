...
///////////////////////////////////////////////////////////////////////////////////////////////
    function get_data_document($q, $offset, $rows){
        $result = array();
        $query = $this->db->query("SELECT COUNT(*) AS Total FROM document_all WHERE title LIKE '%$q%'");
        $row = $query->fetchAll();
        $query = $this->db->query("SELECT id, title, type, category FROM document_all WHERE title LIKE '%$q%'
    ...
    function get_data_document_total($q){
        $query = $this->db->query("SELECT COUNT(*) AS Total FROM document_all WHERE title LIKE '%$q%'");
        $row = $query->fetchAll();
        return $row[0]['Total'];
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////
    function get_info_document($id, $type){
        if($type == 1){ // van ban den
            $query = $this->db->query("SELECT cate_id, file, 'document_in' AS folder FROM tbl_document_in WHERE id = $id");
        }else{ // vn ban di
            $query = $this->db->query("SELECT cate_id, file, 'document_out' AS folder FROM tbl_document_out WHERE id = $id");
        }
        return $query->fetchAll();
    }

    function updateObj_by_code($code, $data){
        $query = $this->update("tbl_works", $data, "code = $code");
        return $query;
    }
}
?>