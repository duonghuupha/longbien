...
class Proof extends Controller{
    ...
    function add(){
        $code = time(); $criteria = $_REQUEST['criteria_id']; $codeproof = strtoupper($_REQUEST['code_proof']);
        $title = $_REQUEST['title']; $createat = date("Y-m-d H:i:s"); $status = 1; $docid  = $_REQUEST['doc_id'];
        ...
        if($this->model->dupliObj(0, $codeproof) > 0){
            ...
            $data = array("code" => $code, "code_proof" => $codeproof, "criteria_id" => $criteria,
                            ...
                            "user_id" => $this->_Info[0]['id'], "status" => $status, 'file_link' => $docid);
    ...
    function update(){
        $id = $_REQUEST['id']; $criteria = $_REQUEST['criteria_id']; $codeproof = strtoupper($_REQUEST['code_proof']);
        $title = $_REQUEST['title']; $createat = date("Y-m-d H:i:s"); $code = $_REQUEST['code'];
        $file = (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') ? $this->_Convert->convert_file($_FILES['file']['name'],'proof') : $_REQUEST['file_old'];
        ...
        if($this->model->dupliObj($id, $codeproof) > 0){
            ...
            $data = array("code_proof" => $codeproof, "criteria_id" => $criteria,
                            "title" => $title, "file" => $file, "create_at" => $createat,
                            "user_id" => $this->_Info[0]['id'], "file_link" =>  $docid);
