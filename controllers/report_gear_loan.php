...
class Report_gear_loan extends Controller{
    ...
    function export_xlsx(){
		...
		$sheet->setCellValue ( 'A3', 'BÁO CÁO THEO DÕI MƯỢN - TRẢ ĐỒ DÙNG');
        ...
        $colIndex = $helpExport->setValueForSheet ( $sheet, $colIndex . $rowIndex, 'Số  lượng đồ dùng mượn', $colIndex );
    ...
    function export_detail(){
		...
		$sheet->mergeCellsByColumnAndRow ( 0, 1, 3, 1 );
		$helpExport->setStyle_12_TNR_B_L ( $sheet, 'A1', 'A1' );
		$sheet->setCellValue ( 'A3', 'BÁO CÁO THEO DÕI MƯỢN - TRẢ ĐỒ DÙNG CHI TIẾT');
		$sheet->mergeCellsByColumnAndRow ( 0, 3, 6, 3 );
		$helpExport->setStyle_14_TNR_B_C ( $sheet, 'A3', 'A3' );
        ...
        foreach($jsonObj as $rows){
            ...
            $json = $this->_Data->get_gear_selected_report($rows['code']); $i = 0;
            ...
            //$objPHPExcel->getActiveSheet()->getStyle($colIndex.$rowIndex.':'.$colIndex.$rowIndex)->getAlignment()->setWrapText(true);
                $colIndex = $helpExport->setValueForSheet ( $sheet, $colIndex . $rowIndex, $i, $colIndex );
                $colIndex = $helpExport->setValueForSheet ( $sheet, $colIndex . $rowIndex, $item['code_u'], $colIndex );
                $colIndex = $helpExport->setValueForSheet ( $sheet, $colIndex . $rowIndex, $item['sub_utensils'], $colIndex );
